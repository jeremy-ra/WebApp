...
export class EmployeeCalculate extends Component {
  ...
  handleSubmit(e){
      ...
      if (this.state.typeId === 1) {
          if (isNaN(this.state.absentDays) || this.state.absentDays.length === 0) {
              alert("Invalid input, Absent days must be a number!");
              return false;
          }
      } else if (this.state.typeId === 2) {
          if (isNaN(this.state.workedDays) || this.state.workedDays.length === 0) {
              alert("Invalid input, Worked days must be a number!");
              return false;
          }
      }
...
<div className='form-group col-md-6'>
  ...
  <input type='number' pattern="[0-9]*" className='form-control' id='inputAbsentDays4' onChange={this.handleChange.bind(this)} value={this.state.absentDays} name="absentDays" placeholder='Absent Days' />
</div> :
<div className='form-group col-md-6'>
  <label htmlFor='inputWorkDays4'>Worked Days: </label>
  <input type='number' pattern="[0-9]*" className='form-control' id='inputWorkDays4' onChange={this.handleChange.bind(this)} value={this.state.workedDays} name="workedDays" placeholder='Worked Days' />
</div>
}
</div>
